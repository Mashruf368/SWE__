...
public class Course {
    ...
    public void setStatus(CourseStatus status) {
        this.status = status;
        this.state  = mapsatetostatus(status);
    }
    ...
    private States mapsatetostatus(CourseStatus status){
        switch(status){
            case OPEN:
    ...
    public void setStatusAdminInteractive(CourseStatus newStatus, Scanner scanner) {
        if(!state.setStatusAdminInteractive(this, mapsatetostatus(newStatus), scanner)){
    ...
    public void cancelCourse() {
        setStatus(CourseStatus.CANCELLED);
        // clear enrolled and waitlist, update students
        for (Student s : new ArrayList<>(enrolled)) {
            Mediator.getInstance().dropfromenroll(s, this);
        }
        for (Student s : new ArrayList<>(waitlist)) {
            Mediator.getInstance().dropfromwaitlist(s, this);
        }
        enrolled.clear();
        waitlist.clear();
        ...
        if (!waitlist.isEmpty()) {
            ...
            if (availableSlots > 0) {
                ...
                for (int i = 0; i < promotionCount; i++) {
                    ...
                    enrolled.add(promoted);
                    Mediator.getInstance().promoteRandom(this, promoted);
